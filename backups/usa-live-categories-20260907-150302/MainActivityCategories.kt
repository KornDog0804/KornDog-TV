...
internal fun MainActivity.setupTabs() {
    ...
    fun launchStreamingApp(
        ...
        try {
            // Match the proven ADB behavior:
            // adb shell am start -n package/activity
            //
            // Do NOT add MAIN or LEANBACK categories here. The explicit
            // ComponentName is enough and avoids launcher intent filtering.
            ...
            android.util.Log.e(
                "StreamingLauncher",
                "Launch failed for $displayName: ${e.javaClass.simpleName}: ${e.message}",
                e
            )
    ...
    binding.dashboardSeries.setOnClickListener {
        launchStreamingApp(
            ...
            "com.espn.startup.presentation.StartupActivity",
            "ESPN"
        )
    }
    ...
    binding.dashboardDiscover.setOnClickListener {
        launchStreamingApp(
            ...
            "com.vudu.android.app.TvMainActivity",
            "Fandango at Home"
        )
    }

    binding.dashboardMax.setOnClickListener {
        launchStreamingApp(
            "com.wbd.stream",
            "com.wbd.beam.BeamActivity",
            "HBO Max"
        )
    }

    binding.dashboardPrime.setOnClickListener {
        launchStreamingApp(
    ...
    binding.dashboardYouTube.setOnClickListener {
        launchStreamingApp(
    ...
    binding.dashboardYouTubeMusic.setOnClickListener {
        launchStreamingApp(
            "com.google.android.youtube.tvmusic",
            "com.google.android.apps.youtube.tvmusic.activity.MainActivity",
            "YouTube Music"
        )
    }
    ...
    for (tv in listOf(binding.tabHome, binding.tabLive, binding.tabCatchup, binding.tabSeries, binding.tabFilms, binding.tabFavorites, binding.tabDiscover, binding.tabConcerts, binding.tabDownloads)) {
...
internal fun MainActivity.updateTabStyles(selected: View) {
    for (tv in listOf(binding.tabHome, binding.tabLive, binding.tabCatchup, binding.tabSeries, binding.tabFilms, binding.tabFavorites, binding.tabDiscover, binding.tabConcerts, binding.tabDownloads)) {
        ...
        val (labelId, iconId, indicatorId) = when (tv.id) {
            ...
            R.id.tabFavorites -> Triple(R.id.tabFavoritesLabel, R.id.tabFavoritesIcon, R.id.tabFavoritesIndicator)
...
internal fun MainActivity.activeSideMenuRow(): View = when {
    ...
    showingFavorites -> binding.navFavorites
...
internal fun MainActivity.updateSideMenuSelection(): View {
    ...
    for (row in listOf(
        ...
        binding.navFilms, binding.navFavorites, binding.navDiscover, binding.navDownloads
    )) {
