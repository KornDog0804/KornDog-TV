...
internal suspend fun MainActivity.classifyAndShow(preserveUi: Boolean = false) {
    ...
    if (hasContent) {
        ...
        if (preserveUi) {
            ...
            if (showingHome) selectHome() else if (showingFavorites) selectFavorites() else if (showingDiscover) selectDiscover() else selectTab(activeTab)
        }
    } else {
        showEmptyState()
    }
    ...
    scope.launch {
        ...
        if (hasContent) {
            ...
            when {
                ...
                showingFavorites -> selectFavorites()
...
internal fun MainActivity.deriveFilmsSeries() {
    ...
    filmsSeriesDeriveJob = scope.launch(Dispatchers.Default) {
        ...
        withContext(Dispatchers.Main) {
            ...
            // Provider-backed Movies / Series finish deriving after the fast Live pass.
            // If Favorites owns the screen, refresh it in-place when those items land
            // instead of switching destinations.
            if (showingFavorites) {
                refreshFavoritesTab()
            }
        }
