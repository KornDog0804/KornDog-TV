...
class MainActivity : AppCompatActivity() {
    ...
    // Favorites intentionally has its own adapter even though it shares the
    // Discover RecyclerView. Favorites may contain LIVE, provider VOD and TMDB
    // items together, so its click path must be the universal Home handler.
    internal val favoritesGridAdapter =
        com.lumora.adapter.PosterGridAdapter(
            onItemLongClick = { item ->
                toggleFavoriteVodItem(item)
                if (showingFavorites) refreshFavoritesTab()
            }
        ) { item ->
            onHomeItemClick(item)
        }
    ...
    private fun activeContentList(): RecyclerView = when {
        ...
        showingFavorites -> binding.discoverGrid
    ...
    private fun isAtSectionTop(): Boolean {
        ...
        if (showingCatchup || showingFavorites || showingDiscover || showingDownloads) return true
    ...
    private fun goToSectionTop() {
        ...
        if (showingCatchup || showingFavorites || showingDiscover || showingDownloads || binding.categorySidebar.visibility != View.VISIBLE) {
    ...
    private fun isTabDrilledIn(): Boolean =
        !showingHome && !showingFavorites && !showingDiscover && !showingDownloads && !showingCatchup && activeTab != 0 &&
    ...
    override fun dispatchKeyEvent(event: android.view.KeyEvent): Boolean {
        ...
        // ── DIRECT 15-SECOND TV SEEK ─────────────────────────────
        //
        // Rewind / Fast Forward are playback commands, not UI-navigation commands.
        // They work immediately without first revealing the control/progress overlay.
        //
        // VOD only. Live TV stays untouched because seeking a live stream can have
        // provider-specific DVR/timeshift semantics and must not inherit VOD behavior.
        if (
            isPlayerVisible &&
            !isPlayerSideMenuOpen() &&
            !upNextActive &&
            ...
            val seekDeltaMs = when (event.keyCode) {
                // Dedicated transport buttons always seek.
                ...
                // DPAD left/right are blind seek shortcuts ONLY while
                // the controls are hidden. Once controls are visible,
                // they belong to normal TV focus navigation so Audio,
                // Subtitles, Versions, etc. remain reachable.
                android.view.KeyEvent.KEYCODE_DPAD_RIGHT ->
                    if (!controlsVisible) 15_000L else 0L
            ...
            if (seekDeltaMs != 0L) {
                ...
                val current = playerManager.currentPosition

                if (
                    current != androidx.media3.common.C.TIME_UNSET &&
                    current >= 0L
                ) {
                    val target = if (
                        ...
                        (current + seekDeltaMs).coerceIn(0L, duration)
                    } else {
                        (current + seekDeltaMs).coerceAtLeast(0L)
                    }
                    ...
                    android.util.Log.d(
                        "LumoraPlayback",
                        "Direct seek ${if (seekDeltaMs > 0) "+15s" else "-15s"}: $current -> $target"
                    )
                }
