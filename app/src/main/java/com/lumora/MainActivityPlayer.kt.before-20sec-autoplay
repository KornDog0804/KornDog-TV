...
internal fun MainActivity.setupPlayerControls() {
    ...
    binding.btnFastForward.setOnClickListener { playerManager.seekBy(15_000); showControls() }
    ...
    fun livePlaybackLog(message: String) {
        val channel = nowPlayingChannel
        if (channel?.mediaType != MediaType.LIVE) return

        runCatching {
            val now = System.currentTimeMillis()
            val version = currentVersionGroup.getOrNull(currentVersionIndex)
            ...
            java.io.File(
                ...
                "$now: $message " +
                    ...
                    "version=${version?.name ?: channel.name} " +
                    "idx=$currentVersionIndex/${currentVersionGroup.size} " +
                    "played=$currentStreamPlayed " +
                    "pos=${playerManager.currentPosition} " +
                    "url=${playerManager.currentMediaUri() ?: version?.url ?: channel.url}\n"
            )
        }
    }
    ...
    playerManager.addListener(object : Player.Listener {
        override fun onPlaybackStateChanged(state: Int) {
            ...
            livePlaybackLog("STATE=$stateName")
            ...
            if (state == Player.STATE_READY || state == Player.STATE_ENDED) {
                ...
                if (state == Player.STATE_READY) {
                    // READY happens again after ordinary seeks. Fast-forwarding or
                    // rewinding must never reopen the Start Over / Resume dialog.
                    //
                    // Only the first READY transition for this playback attempt is
                    // eligible to ask about a previously saved resume position.
                    val firstReadyForThisAttempt = !currentStreamPlayed
                    currentStreamPlayed = true

                    if (firstReadyForThisAttempt) {
        ...
        override fun onPlayerError(error: PlaybackException) {
            ...
            // A live HLS playlist is a moving window. Falling behind that window
            // does NOT mean the channel or provider is dead. Re-open the exact
            // currently selected live version so Media3 joins the new live edge.
            //
            // This must happen BEFORE quality/version failover. Otherwise a healthy
            // channel gets incorrectly marked/fallen away from simply because its
            // old HLS segments aged out of the playlist.
            if (
                nowPlayingChannel?.mediaType == MediaType.LIVE &&
                error.errorCode == PlaybackException.ERROR_CODE_BEHIND_LIVE_WINDOW
            ) {
                val liveVersion =
                    currentVersionGroup.getOrNull(currentVersionIndex)
                        ?: nowPlayingChannel

                if (liveVersion != null && liveVersion.url.isNotBlank()) {
                    android.util.Log.w(
                        "LumoraPlayback",
                        "Behind live window; rejoining current live edge " +
                            "channel=${liveVersion.name} " +
                            "version=${currentVersionIndex + 1}/${currentVersionGroup.size}"
                    )

                    livePlaybackLog(
                        "BEHIND_LIVE_WINDOW -> REJOIN SAME VERSION"
                    )
...
internal fun MainActivity.attemptBufferFailover() {
    ...
    val ageMs = System.currentTimeMillis() - currentStreamStartMs

    if (withinFailoverGrace()) {
        runCatching {
            java.io.File(
                android.os.Environment.getExternalStoragePublicDirectory(
                    android.os.Environment.DIRECTORY_DOWNLOADS
                ...
                "${System.currentTimeMillis()}: BUFFER_FAILOVER_SKIPPED_GRACE " +
                    "ageMs=$ageMs stalls=${stallTimestamps.size}\n"
            )
        }
    ...
    runCatching {
        java.io.File(
            ...
            "${System.currentTimeMillis()}: BUFFER_FAILOVER_TRIGGERED " +
                "ageMs=$ageMs stalls=${stallTimestamps.size}\n"
        )
    }
...
internal fun MainActivity.hidePlayer() {
    ...
    // activeTab may still be 0 simply because Live is the app's default tab.
    // That does NOT mean playback is returning to Live. A movie/episode commonly
    // returns to its detail page while activeTab is still 0.
    //
    // Never resurrect the Live preview unless the real Live pane owns the screen.
